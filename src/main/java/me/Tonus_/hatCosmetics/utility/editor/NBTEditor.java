...
public class NBTEditor {
    ...
    static abstract class BaseEditor<T, E extends BaseEditor<T, E>> {
        private final Plugin plugin;
        ...
        public <P, C> E addTag(PersistentDataType<P, C> type, String key, C val) {
            if (pdc != null) pdc.set(new NamespacedKey(plugin, key), type, val);
            return self();
        }
        ...
        public E removeTag(String key) {
            if (pdc != null) pdc.remove(new NamespacedKey(plugin, key));
            return self();
        }
        ...
        /**
         * Gets the value of a tag from the persistent data container, if it exists.
         *
         * @param <P>  The primitive type of the tag.
         * @param <C>  The complex type of the tag.
         * @param type The persistent data type.
        ...
        public <P, C> Optional<C> getTag(PersistentDataType<P, C> type, String key) {
            if (pdc == null || pdc.isEmpty()) return Optional.empty();
            var val = pdc.get(new NamespacedKey(plugin, key), type);
    ...
    public static class ItemStackEditor extends BaseEditor<ItemStack, ItemStackEditor> {
        ...
        public ItemStack apply() {
            if (item != null && meta != null) item.setItemMeta(meta);
            return item;
        }
    ...
    public @NotNull ItemStackEditor of(@Nullable ItemStack item) {
        if (item == null) return new ItemStackEditor(this.plugin, null, null, null);
    ...
    public @NotNull ItemMetaEditor of(@Nullable ItemMeta meta) {
        if (meta == null) return new ItemMetaEditor(this.plugin, null, null);
